...
from db import (
    get_stats, get_all_users, update_subscription, get_user,
    DATABASE_PATH
)
        ...
        for user_id in users:
            ...
            if (sent + failed) % 10 == 0:
                await status_msg.edit_text(
                    f"🔄 Рассылка: {sent} успешно, {failed} ошибок из {len(users)}"
                )
...
async def cmd_extend(message: Message):
    ...
    try:
        ...
        await message.answer(
            f"✅ Подписка пользователя {user_id} продлена на {days} дней\n"
            f"📅 Новая дата окончания: {new_end_date}"
        )
        
        logger.info(f"Admin {message.from_user.id} extended subscription for user {user_id} by {days} days")
        ...
        logger.error(f"Error in cmd_extend: {e}")
...
async def cmd_revoke(message: Message):
    ...
    try:
        ...
        user = await get_user(user_id)
        if not user:
            await message.answer(f"�� Пользователь {user_id} не н��йден")
            return
        
        # Устанавливаем подписку в прошлое (сегодня - 1 день)
        ...
        logger.info(f"Admin {message.from_user.id} revoked subscription for user {user_id}")
        ...
        logger.error(f"Error in cmd_revoke: {e}")
