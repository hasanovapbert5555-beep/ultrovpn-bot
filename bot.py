...
async def show_referral(message: Message):
    try:
        ...
        await message.answer("❌ Ошибка при получении реферальной информации")
...
async def gift_subscription(message: Message):
    try:
        ...
        logger.error(f"Error in gift_subscription: {e}")
...
async def connect_vpn(message: Message, state: FSMContext):
    try:
        ...
        logger.error(f"Error in connect_vpn: {e}")
        await message.answer("❌ Ошибка при подключении VPN")
...
async def process_device_name(message: Message, state: FSMContext):
    try:
        ...
        if not all(c.isalnum() or c in ' -' for c in device_name):
        ...
        if config:
            ...
            await message.answer(text, parse_mode="HTML")
            logger.info(f"✅ Generated VPN config for user {message.from_user.id}: {device_name}")
        else:
            await message.answer(
        ...
        logger.error(f"Error in process_device_name: {e}")
        await message.answer("❌ Ошибка при обработке названия устройства")
        await state.clear()
...
async def process_tariff(callback: CallbackQuery):
    try:
        ...
        logger.error(f"Error in process_tariff: {e}")
...
async def main():
    try:
        ...
        setup_success = await xray_manager.setup_reality_inbound()
        if not setup_success:
            logger.error("❌ Failed to setup REALITY inbound")
            return
        
        logger.info("✅ REALITY inbound configured")
