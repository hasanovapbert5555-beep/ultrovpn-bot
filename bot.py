...
Функции: VPN + Антиглушилка + Партнёрка + API + Промокоды + Стриминг + Админка + Аналитика + Автопостинг
Версия: ПОЛНАЯ — С АНАЛИТИКОЙ И АВТОПОСТИНГОМ
"""
...
async def check_subscription(user_id):
    ...
    try:
        cm = await bot.get_chat_member(f"@{REQUIRED_CHANNEL}", user_id)
...
async def cmd_start(message: Message):
    ...
    if not ok:
        await message.reply(err, parse_mode='Markdown', reply_markup=sub_keyboard())
        return
...
async def add_device_cb(callback: CallbackQuery):
    ...
    if not ok:
        await callback.message.reply(f"🔒 ПОДПИШИСЬ: {REQUIRED_CHANNEL_LINK}")
        return
